...
// ── Dashboard ──
function renderDashboard({ event, alerts, zones, chartData }) {
  const activeAlerts = (alerts || state.alerts || []).filter(a => a.status === 'active');
  const zoneList = zones || state.zones || [];
  const cd = chartData || state.chartData || {};
...
<div class="page-enter">
  <div class="page-hdr">
    <div class="ph-left">
      ...
      <p>${event?.venue || 'Wankhede Stadium'} · ${event?.name || 'MI vs RCB IPL 2026'} · Over ${event?.over || 44}</p>
    </div>
    <div class="ph-right">
  ...
  <div class="metrics">
    <div class="metric">
      ...
      <div class="metric-value text-grn" id="att-val">${(event?.attendance || 0).toLocaleString('en-IN')}</div>
      <div class="metric-delta"><span class="delta-up">+340</span>&nbsp;last 5 min · ${Math.round((event?.attendance || 0) / (event?.capacity || 80000) * 100)}% capacity</div>
    </div>
    <div class="metric">
      <div class="metric-label">Active alerts</div>
      <div class="metric-value text-red">${activeAlerts.length}</div>
      <div class="metric-delta delta-dn">${activeAlerts.filter(a => a.severity === 'high').length} high · ${activeAlerts.filter(a => a.severity === 'medium').length} medium</div>
    </div>
    <div class="metric">
    ...
    <div class="metric">
      ...
      <div class="metric-value">${zoneList.filter(z => z.status === 'critical' || z.status === 'high').length}<span style="font-size:16px;color:var(--t3);font-weight:400"> / ${zoneList.length}</span></div>
...
<div class="page-enter">
  <div class="page-hdr">
    <div class="ph-left"><h1>Live heatmap</h1><p>Real-time density across all ${zoneList.length} venue zones</p></div>
    <div class="ph-right">
...
function renderAlerts({ alerts }) {
  ...
  window._currentAlertFilter = window._currentAlertFilter || 'active';
  return `
<div class="page-enter">
  <div class="page-hdr">
...
function filterAlerts(filter, btn) {
  window._currentAlertFilter = filter;
  document.querySelectorAll('#af-active,#af-resolved,#af-all').forEach(b => b.classList.remove('active'));
  if (btn) btn.classList.add('active');
  const alerts = filter === 'all' ? (state.alerts || []) : (state.alerts || []).filter(a => a.status === filter);
...
async function resolveAlert(id) {
  try {
    ...
    if (alert) {
      ...
      showToast(`"${alert.title}" marked resolved`, 'success');
    }
...
async function addAlert() {
  try {
    const newAlert = await api.post('/api/alerts', {});
    if (state.alerts) state.alerts.unshift(newAlert);
...
function renderFacilities({ facilities }) {
  const facList = facilities || state.facilities || [];
  window._currentFacilityFilter = window._currentFacilityFilter || 'all';
  return `
<div class="page-enter">
  <div class="page-hdr">
...
function filterFacilities(type, btn) {
  window._currentFacilityFilter = type;
  document.querySelectorAll('.facility-filters .filter-btn').forEach(b => b.classList.remove('active'));
  if (btn) btn.classList.add('active');
  const facs = type === 'all' ? (state.facilities || []) : (state.facilities || []).filter(f => f.type === type);
...
function renderStaff({ staff }) {
  const staffList = staff || state.staff || [];
  return `
<div class="page-enter">
  <div class="page-hdr">
...
function renderStaffRows(staffList) {
  document.getElementById('staff-body').innerHTML = (staffList || []).map(s => `
    <tr>
      <td>
...
function filterStaff(q) {
  const filtered = q ? (state.staff || []).filter(s => s.name.toLowerCase().includes(q.toLowerCase()) || s.role.toLowerCase().includes(q.toLowerCase())) : (state.staff || []);
...
<div class="page-enter">
  ...
  <div class="grid2">
    ...
    <div class="card">
      ...
      ${crowdAI.map(r => `<div class="ai-card" style="background:${r.bg};border:1px solid ${r.border}">
...
// ── Queue Manager ──
function renderQueues({ gates, concessions }) {
  const gateList = gates || state.gateData || [];
  const concList = concessions || state.concessions || [];
...
<div class="page-enter">
  ...
  <div class="grid2">
    ...
    <div class="card">
      ...
      <div id="exit-phases">
        ${exitPhases.map(p => `<div class="exit-phase-row" id="ep-${p.time.replace(':', '')}">
...
// ── Analytics ──
function renderAnalytics({ kpis, chartData }) {
...
<div class="page-enter">
  ...
  <div class="grid3 mb-12">
    ${analyticsKPIs.map(k => `<div class="metric">
  ...
  <div class="grid2 mb-12">
    ...
    <div class="card">
      ...
      <div style="font-size:11px;color:var(--t3);margin-bottom:10px">Current state of all ${zones.length} venue zones</div>
      <div style="position:relative;height:240px">
        <canvas id="chart-donut"></canvas>
        <div class="donut-center">
          <div class="donut-num">${zones.length}</div>
