...
function showPage(id, el) {
  ...
  main.innerHTML = PAGE_RENDERERS[id] ? PAGE_RENDERERS[id]() : '<p>Page not found</p>';
  // Post-render hooks
  requestAnimationFrame(() => {
    ...
    if (id === 'staff') {
      renderStaffRows(RoaryData.staff);
    }
    if (id === 'queues') {
      gateData = JSON.parse(JSON.stringify(RoaryData.gates));
      renderBarList(gateData, 'gate-bars');
      renderBarList(RoaryData.concessions, 'conc-bars');
    }
    if (id === 'analytics') {
...
let attVal = RoaryData.event.attendance;
setInterval(() => {
  if (Math.random() > 0.6) {
    attVal += Math.floor(Math.random() * 4);
    const el = document.getElementById('att-val');
    if (el) el.textContent = attVal.toLocaleString('en-IN');
  }
}, 2500);
